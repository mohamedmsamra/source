...
public class SpringUtilities {
	...
	/**
	 * * Aligns the first rows * cols * components of parent in * a grid. Each
	...
	public static void makeCompactGrid(Container parent, int rows, int cols,
		...
		try {
			...
			return; // Align all cells in each column and make them the same
						// width.
		}
		...
		for (int c = 0; c < cols; c++) {
			...
			for (int r = 0; r < rows; r++) {
				width = Spring.max(width, SpringUtilities.getConstraintsForCell(r,
				      c, parent, cols).getWidth());
			}
			for (int r = 0; r < rows; r++) {
				SpringLayout.Constraints constraints = SpringUtilities
		...
		for (int r = 0; r < rows; r++) {
			...
			for (int c = 0; c < cols; c++) {
				height = Spring.max(height, SpringUtilities.getConstraintsForCell(
